...
### Added
- **Forgot & Reset Password (WIQ-V1-015)** — self-service password recovery built on the current auth stack. Backend adds `POST /auth/forgot-password` (enumeration-safe generic response, per-IP rate limited via the existing limiter, delivery dispatched as a background task) and `POST /auth/reset-password` (signed short-lived JWT with `purpose: "password_reset"` and a password-hash fingerprint claim for single-use semantics — no migration or token store; rejects reuse of the current password; revokes every refresh session; records `password_reset` / `password_reset_email_sent` audit events). Reset emails reuse the provider-based email service (`EMAIL_BACKEND=console`/`smtp`) with a new template and links built from `FRONTEND_URL`; tokens are never logged or returned by the API. Frontend adds `/forgot-password` and `/reset-password?token=…` pages (validation, loading/success/error states, rate-limit messaging), plus a "Forgot password?" link on the login page.
- **Email Verification (#57)** — account emails are now verified via signed one-time tokens. Backend adds `users.email_verified_at` (nullable) with an `email_verified` property, a provider-based email service (`app/services/email.py`: `console` backend for development that keeps an in-process outbox + redacted logs, `smtp` backend for production with STARTTLS), a Jinja2 verification template, `POST /auth/verify-email` (idempotent; generic `400` for every unusable token) and `POST /auth/resend-verification` (enumeration-safe, per-IP rate limited, always `200`); registration dispatches a verification link immediately and delivery failures never block sign-up. `GET /auth/me` and every auth response now expose `email_verified` / `email_verified_at`. Audit events: `verification_email_sent` and `email_verified`. Frontend adds a dashboard resend banner for unverified users and a `/verify-email` page (works for logged-in and logged-out users) that verifies the token, refreshes the cached profile, and offers resend with rate-limit messaging.
- **Notification & Communication System (#14)** — centralized, database-backed notifications for all four roles. New backend stack: `notifications` table (`backend/app/models/notification.py`), repository layer, Pydantic schemas, `NotificationService`/`NotificationDispatcher`/`NotificationBroadcaster` services, and pure `notification_formatters` helpers; the dispatcher hooks into pickup lifecycle events (`pickup_created/accepted/started/collected/completed`), dealer profile submit/approve/reject, inventory lot create/reserve/cancel/purchase/expire, and emits `system` notifications; admins broadcast to roles via `POST /admin/notifications/broadcast`. API surface: `GET /notifications` (paginated + status filter), `GET /notifications/unread`, `GET /notifications/unread/count`, `GET /notifications/{id}`, `POST /notifications/{id}/read`, `POST /notifications/read-all`, `DELETE /notifications/{id}`, `DELETE /notifications/read`. Frontend adds a server-backed NotificationsCenter (`links` deep-link to pickups/marketplace), a header bell with a live unread badge + dropdown (mark-all-read, preview list, view-all), notifications pages for every portal (`/{role}/notifications`), filter tabs (All/Unread/Read), pagination, and optimistic read/delete mutations (TanStack Query). Legacy localStorage citizen notifications are untouched.
- **Collector Live Map & Route Tracking (#13)** — new `/collector/map` dashboard page with a dependency-free SVG live map (auto-fitting equirectangular projection), an interactive pickup-marker layer, and a full route-tracking suite: collectors report their position via browser geolocation (`POST /collector/location`, backed by new `collector_locations` + `collector_location_history` tables), view a sequenced nearest-neighbour route over their assigned pickups (`GET /collector/route`), browse the nearest pending requests with Haversine distance (`GET /collector/nearby-pickups`), and pull step-by-step navigation with travel-time estimates (`GET /collector/navigation/{pickup_id}`). The backend exposes a single combined payload endpoint (`GET /collector/map`) so the page renders in one round trip; ETAs use a `DEFAULT_ROUTE_SPEED_KMPH` constant for deterministic estimates without a mapping service.
- **Marketplace Purchase & Reservation Flow** — complete buy-side marketplace for approved dealers under `/marketplace/*`: paginated inventory browsing with search/sort/filter, lot detail, 24-hour reservations with cancellation, and row-lock-guarded purchase that creates a `marketplace_orders` record and a `marketplace_transactions` financial ledger (reservation / cancellation / reservation_expired / purchase); expired reservations are auto-released and recorded as `reservation_expired` transactions; dealers get a new frontend suite — marketplace grid/table with filters (`/dealer/marketplace`), lot detail page, and order + transaction history (`/dealer/orders`) — all gated by `DealerApprovalGate
