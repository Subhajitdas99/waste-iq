...
def test_create_pickup_request_success(client, citizen_headers, valid_pickup_payload):
...
def test_create_pickup_request_without_photo_succeeds(
...
def test_collector_cannot_create_pickup_request(client, collector_headers, valid_pickup_payload):
    response = client.post("/pickup-requests", data=valid_pickup_payload, headers=collector_headers)
    assert response.status_code == 403


def test_create_pickup_request_requires_auth(client, valid_pickup_payload):
...
def test_list_pickup_requests_returns_own_requests_only(
    client, citizen_headers, citizen_user, db_session, valid_pickup_payload
):
...
def test_get_pickup_request_detail_includes_timeline(client, citizen_headers, valid_pickup_payload):
    created = client.post(
...
def test_citizen_cannot_view_other_citizens_request(
    client, citizen_headers, make_user, auth_headers, valid_pickup_payload
):
    other_citizen = make_user(role=UserRole.citizen, email="other@wasteiq.test", phone="9000099999")
...
def test_update_pending_pickup_request_success(client, citizen_headers, valid_pickup_payload):
    created = client.post(
        "/pickup-requests", data=valid_pickup_payload, headers=citizen_headers
    ).json()
...
def test_update_pickup_request_cannot_set_status_directly(
    client, citizen_headers, valid_pickup_payload
):
    """
...
def test_cancel_pending_pickup_request_success(client, citizen_headers, valid_pickup_payload):
    created = client.post(
        "/pickup-requests", data=valid_pickup_payload, headers=citizen_headers
    ).json()
...
def test_cancel_already_accepted_pickup_fails(
...
def test_cancel_other_citizens_pickup_fails(
    client, citizen_headers, make_user, auth_headers, valid_pickup_payload
):
    other_headers = auth_headers(
        make_user(role=UserRole.citizen, email="other2@wasteiq.test", phone="9000088888")
    )
    created = client.post(
        "/pickup-requests", data=valid_pickup_payload, headers=other_headers
    ).json()
...
def test_collector_cannot_cancel_pickup_request(
    client, citizen_headers, collector_headers, valid_pickup_payload
):
    created = client.post(
        "/pickup-requests", data=valid_pickup_payload, headers=citizen_headers
...
def test_citizen_summary_counts(client, citizen_headers, valid_pickup_payload):
    client.post("/pickup-requests", data=valid_pickup_payload, headers=citizen_headers)
    second = client.post(
        "/pickup-requests", data=valid_pickup_payload, headers=citizen_headers
    ).json()
...
def test_create_pickup_request_with_sprint5_details(client, citizen_headers, valid_pickup_payload):
...
def test_create_pickup_request_without_details_returns_nulls(
    client, citizen_headers, valid_pickup_payload
):
    response = client.post("/pickup-requests", data=valid_pickup_payload, headers=citizen_headers)
    assert response.status_code == 201
...
def test_create_pickup_request_rejects_invalid_weight(
    client, citizen_headers, valid_pickup_payload
):
    response = client.post(
        "/pickup-requests",
...
def test_create_pickup_request_rejects_overlong_notes(
    client, citizen_headers, valid_pickup_payload
):
    response = client.post(
        "/pickup-requests",
...
def test_list_pickup_requests_returns_sprint5_details(
    client, citizen_headers, valid_pickup_payload
):
    created = client.post(
        "/pickup-requests",
