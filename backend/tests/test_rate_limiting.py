...
def test_429_does_not_leak_account_existence(client, monkeypatch):
    ...
    assert ghost_body == known_body, "429 bodies must be identical for existing and unknown emails"
...
def test_lockout_does_not_reveal_account_existence(client, db_session, monkeypatch):
    ...
    ghost_body.get("error", {}).pop("request_id", None)
    assert locked_body == ghost_body, "locked and unknown accounts must produce identical responses"
