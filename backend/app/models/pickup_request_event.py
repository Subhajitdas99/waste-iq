...
class PickupRequestEvent(Base):
    ...
    request_id: Mapped[int] = mapped_column(ForeignKey("pickup_requests.id", ondelete="CASCADE"), nullable=False, index=True)
    actor_id: Mapped[int | None] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
    status: Mapped[PickupStatus] = mapped_column(Enum(PickupStatus, native_enum=False), nullable=False, index=True)
